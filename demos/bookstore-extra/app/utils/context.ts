...
import type { User, Cart } from '../../services.ts'
...
export function getCurrentCart(): Cart {
  let sessionCart = getContext().session.get('cart')
  // Simple validation/defaulting here since we can't easily inject CartService into this utility
  // without making it async or complex.
  if (
    typeof sessionCart === 'object' &&
    sessionCart !== null &&
    'items' in sessionCart &&
    Array.isArray((sessionCart as any).items)
  ) {
    return sessionCart as Cart
  }
  return { items: [] }
}
