...
import '../implementation/setup.ts'
import { loginAsAdmin, requestWithSession } from '../test/helpers.ts'
import { router } from './router.ts'
import { booksData } from '../implementation/db.ts'
...
describe('uploads handler', () => {
  it('serves uploaded files from storage', async () => {
    ...
    let initialBookCount = booksData.length
    ...
    assert.equal(booksData.length, initialBookCount + 1)

    let newBook = booksData[booksData.length - 1]
